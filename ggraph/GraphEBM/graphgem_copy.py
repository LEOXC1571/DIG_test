...
class GraphEBM(Generator):
    def __init__(self, n_atom, n_atom_type, n_edge_type, hidden_size, device=None):
        super(GraphEBM, self).__init__()
        if device is None:
            self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        else:
            self.device = device
        self.energy_function = EnergyFunc(n_atom_type, hidden_size, n_edge_type).to(self.device)
        self.n_atom = n_atom
    ...
    def train_rand_gen(self, loader, lr, wd, max_epochs, c, ld_step, ld_noise, ld_step_size, clamp, alpha,
        ...
        for epoch in range(max_epochs):
            ...
            for _, batch in enumerate(tqdm(loader)):
                ...
                noise_x = torch.rand_like(neg_x, device=self.device)
                noise_adj = torch.rand_like(neg_adj, device=self.device)
            ...
            if (epoch + 1) % save_interval == 0:
                ...
                print('=====================================')
            print('Epoch: {:03d}, Loss: {:.6f}, Energy Loss: {:.6f}, Regularizer Loss: {:.6f}, Sec per Epoch: {:.2f}'.format(
                epoch + 1, (sum(losses) / len(losses)).item(), (sum(losses_en) / len(losses_en)).item(),
                (sum(losses_reg) / len(losses_reg)).item(), t_end - t_start
            ))
            print('=====================================')

    def run_rand_gen(self, ckpt_path, n_samples, c, ld_step, ld_noise, ld_step_size, clamp, atomic_num_list):
        print(('Loading parameters from {}'.format(ckpt_path)))
        self.energy_function.load_state_dict(torch.load(ckpt_path))
        parameters = self.energy_function.parameters()
    ...
    def train_goal_directed(self, loader, lr, wd, max_epochs, c, ld_step, ld_noise, ld_step_size, clamp,
                            alpha, save_interval, save_dir):
        parameters = self.energy_function.parameters()
        optimizer = Adam(parameters, lr=lr, betas=(0.0, 0.999), weight_decay=wd)
        ...
        for epoch in range(max_epochs):
            ...
            if (epoch + 1) % save_interval == 0:
                ...
                print('=====================================')
            print(
                'Epoch: {:03d}, Loss: {:.6f}, Energy Loss: {:.6f}, Regularizer Loss: {:.6f}, Sec per Epoch: {:.2f}'.format(
                    epoch + 1, (sum(losses) / len(losses)).item(), (sum(losses_en) / len(losses_en)).item(),
                    (sum(losses_reg) / len(losses_reg)).item(), t_end - t_start
                ))
            print('=====================================')

    def run_prop_opt(self, ckpt_path, init_loader, c, ld_step, ld_noise, ld_step_size,
                     clamp, atomic_num_list,train_smiles):
        print("Loading parameters from {}".format(ckpt_path))
        self.energy_function.load_state_dict(torch.load(ckpt_path))
        parameters = self.energy_function.parameters()
        ...
        for _, batch in enumerate(tqdm(init_loader)):
